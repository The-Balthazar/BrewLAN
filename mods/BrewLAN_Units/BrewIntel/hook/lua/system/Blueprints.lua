...
function RebalanceExistingIntel(all_bps)
    ...
    for id, omniID in pairs(t3radars) do
        local bp = all_bps[id]
        local omnibp = all_bps[omniID]

        if _VERSION ~= "Lua 5.0.1" then
            if bp then bp.WikiInfoboxNote = '<LOC wiki_infobox_stats_note>Note: This unit has its stats redefined<br />at the start of the game.' end
            if omnibp then omnibp.WikiInfoboxNote = '<LOC wiki_infobox_stats_note>Note: This unit has its stats redefined<br />at the start of the game.' end
        end

        if bp and omnibp and bp.Intel.OmniRadius and bp.Intel.OmniRadius ~= 0 and bp.Economy.MaintenanceConsumptionPerSecondEnergy and bp.Economy.BuildCostEnergy and bp.Economy.BuildCostMass and bp.Economy.BuildTime then
            --Scale omni radii
