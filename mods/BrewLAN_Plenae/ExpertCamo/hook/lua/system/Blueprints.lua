...
function ModBlueprints(all_blueprints)
    ...
    ExtractExpertCamoTerrainMeshes(all_blueprints.Unit)
end
...
function ExtractExpertCamoTerrainMeshes(all_bps)
    --Get textures list
    ...
    if type(textures[1]) == "string" then
        for i, tex in textures do
            --if not string.find(tex, "_lod") or string.find(tex, "_lod0") then
                ...
                --local bp, tt = string.match(tex, "/terrainmeshes/([^_]+)_([^_]+)") -- Unfortunately for diggles code, match is a 5.1 function, and supcom uses 5.0.1
