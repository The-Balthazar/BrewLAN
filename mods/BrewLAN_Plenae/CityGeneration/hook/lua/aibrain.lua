...
AIBrain = Class(oldAIBrain) {
    OnCreateAI = function(self, planName)
        ...
        for armyname, MarkerPos in OpenStartZones do
            self:ForkThread(self.CreateCity, table.copy(MarkerPos))
        end
    end,

    CreateCity = function(self, MarkerPos)
        coroutine.yield(1)
        --------------------------------------------------------------------
                    ...
                    if math.random(1,10) ~= 10 then
                        ...
                        if check then--not (CityData.Grid2Map[x + math.max(0, v[1])] and CityData.Grid2Map[checkX][checkY] then
                            if not CityData.PowerPlant then
                                if CityData.NoGrids > 6 and __blueprints.seb1201 then
                                    CityData.PowerPlant = SafeSpawn('seb1201', {pos[1] + v[1]*3, nil,  pos[3] + v[2]*3})
                                    if not CityData.PowerPlant then return end
                                elseif CityData.NoGrids > 1 then
                                    for _, j in Corners(0.75) do
                                        CityData.PowerPlant = SafeSpawn('ueb1101', {pos[1]+v[1]*3+j[1], nil, pos[3]+v[2]*3+j[2]})
                                        if not CityData.PowerPlant then return end
                                    end
                                else
                                    CityData.PowerPlant = SafeSpawn('ueb1101', {pos[1] + v[1]*3, nil,  pos[3] + v[2]*3})
                                    if not CityData.PowerPlant then return end
                                end
                            else
